...
func (s *AccessService) ListUsers() ([]dto.AccessUserInfo, error) {
	...
	if err := global.DB.Table("rbac_role_bindings AS b").
		Select("b.id, b.user_id, b.role_id, r.key AS role_key, r.name AS role_name, b.scope_type, b.scope_id, b.resource_type").
		Joins("JOIN rbac_roles r ON r.id = b.role_id").
		Order("r.sort ASC, b.scope_type ASC, b.scope_id ASC").
	...
	for _, row := range rows {
		byUser[row.UserID] = append(byUser[row.UserID], dto.AccessBindingInfo{
			...
			ScopeType: row.ScopeType, ScopeID: row.ScopeID, ResourceType: row.ResourceType,
		})
	}
...
func (s *AccessService) CreateUser(req dto.AccessUserCreate) error {
	...
	return global.DB.Transaction(func(tx *gorm.DB) error {
		var count int64
		if err := tx.Model(&model.AccessUser{}).Where("username = ?", username).Count(&count).Error; err != nil {
			return err
		}
		if count != 0 {
		...
		user := model.AccessUser{
			Username: username, DisplayName: strings.TrimSpace(req.DisplayName), Email: strings.TrimSpace(req.Email),
			PasswordHash: string(hash), Status: model.AccessUserStatusActive, AuthSource: "local",
			RequireMFA: req.RequireMFA, PasswordChangedAt: &now,
		}
		if user.DisplayName == "" {
...
func (s *AccessService) UpdateUserStatus(req dto.AccessUserStatusUpdate) error {
	...
	if req.Status == model.AccessUserStatusDisabled {
		...
		if last {
			return errors.New("the last active global administrator cannot be disabled")
		}
	}
	if err := global.DB.Model(&user).Update("status", req.Status).Error; err != nil {
		return err
	}
	if req.Status == model.AccessUserStatusDisabled {
...
func (s *AccessService) ResetUserPassword(req dto.AccessUserPasswordReset) error {
	...
	now := time.Now()
	result := global.DB.Model(&model.AccessUser{}).Where("id = ?", req.ID).Updates(map[string]any{
...
func (s *AccessService) ReplaceUserBindings(req dto.AccessUserBindingsUpdate) error {
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		for _, binding := range bindings {
			...
			if err := tx.First(&role, binding.RoleID).Error; err != nil {
				return err
			}
			if role.Key == rbac.RoleAdministrator && binding.ScopeType == model.AccessScopeGlobal {
				proposedGlobalAdmin = true
			}
		}
		if !proposedGlobalAdmin {
			...
			if last {
				return errors.New("the last active global administrator must keep the administrator role")
			}
		}
		if err := tx.Where("user_id = ?", user.ID).Delete(&model.AccessRoleBinding{}).Error; err != nil {
			return err
		}
		for _, binding := range bindings {
			binding.UserID = user.ID
...
func (s *AccessService) ListRoles() ([]dto.AccessRoleInfo, error) {
	...
	for _, role := range roles {
		...
		if err := global.DB.Table("rbac_permissions AS p").
			Select("p.*").
			Joins("JOIN rbac_role_permissions rp ON rp.permission_id = p.id").
			Where("rp.role_id = ?", role.ID).
			Order("p.resource_type ASC, p.feature ASC, p.action ASC").
		...
		for _, permission := range permissions {
			item.Permissions = append(item.Permissions, dto.AccessPermissionInfo{
				Code: permission.Code, ResourceType: permission.ResourceType, Feature: permission.Feature,
				Action: permission.Action, RiskLevel: permission.RiskLevel, Description: permission.Description,
			})
		}
...
func (s *AccessService) ListProjects() ([]dto.AccessProjectInfo, error) {
	...
	for _, project := range projects {
		...
		if err := global.DB.Where("project_id = ?", project.ID).Order("resource_type ASC, resource_id ASC").Find(&resources).Error; err != nil {
			return nil, err
		}
...
func (s *AccessService) UpdateProject(req dto.AccessProjectUpdate) error {
	return global.DB.Model(&model.AccessProject{}).Where("id = ?", req.ID).Updates(map[string]any{
		"name": strings.TrimSpace(req.Name), "slug": strings.ToLower(strings.TrimSpace(req.Slug)),
		"description": strings.TrimSpace(req.Description), "status": req.Status,
	}).Error
}
...
func (s *AccessService) ReplaceProjectResources(req dto.AccessProjectResourcesUpdate) error {
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("project_id = ?", req.ID).Delete(&model.AccessProjectResource{}).Error; err != nil {
			return err
		}
		seen := make(map[string]struct{})
		for _, input := range req.Resources {
			key := fmt.Sprintf("%d:%s:%s", input.NodeID, input.ResourceType, input.ResourceID)
			if _, exists := seen[key]; exists {
				continue
			}
			seen[key] = struct{}{}
			resource := model.AccessProjectResource{ProjectID: req.ID, NodeID: input.NodeID, ResourceType: input.ResourceType, ResourceID: input.ResourceID}
...
func validateBindings(tx *gorm.DB, inputs []dto.AccessBindingInput) ([]model.AccessRoleBinding, error) {
	...
	for _, input := range inputs {
		var role model.AccessRole
		if err := tx.Where("key = ?", input.RoleKey).First(&role).Error; err != nil {
			return nil, fmt.Errorf("role %s does not exist: %w", input.RoleKey, err)
		}
		scopeType := strings.TrimSpace(input.ScopeType)
		scopeID := strings.TrimSpace(input.ScopeID)
		resourceType := strings.TrimSpace(input.ResourceType)
		if err := validateRoleScope(tx, role.Key, scopeType, scopeID, resourceType); err != nil {
			return nil, err
		}
		key := fmt.Sprintf("%d:%s:%s:%s", role.ID, scopeType, scopeID, resourceType)
		if _, exists := seen[key]; exists {
			continue
		}
		seen[key] = struct{}{}
		result = append(result, model.AccessRoleBinding{RoleID: role.ID, ScopeType: scopeType, ScopeID: scopeID, ResourceType: resourceType})
	}
	return result, nil
}

func validateRoleScope(tx *gorm.DB, roleKey, scopeType, scopeID, resourceType string) error {
	...
	if !ok || !roleScopes[scopeType] {
		return fmt.Errorf("role %s cannot be assigned at %s scope", roleKey, scopeType)
	}
	switch scopeType {
	case model.AccessScopeGlobal:
		if scopeID != "*" || resourceType != "" {
			return errors.New("global scope requires scopeId='*' and no resourceType")
		}
	case model.AccessScopeNode:
		if _, err := strconv.ParseUint(scopeID, 10, 64); err != nil || resourceType != "" {
			return errors.New("node scope requires a numeric scopeId and no resourceType")
		}
	case model.AccessScopeProject:
		projectID, err := strconv.ParseUint(scopeID, 10, 64)
		if err != nil || projectID == 0 || resourceType != "" {
			return errors.New("project scope requires a numeric project scopeId and no resourceType")
		}
		var count int64
		if err := tx.Model(&model.AccessProject{}).Where("id = ?", uint(projectID)).Count(&count).Error; err != nil || count == 0 {
			return errors.New("project scope references an unknown project")
		}
	case model.AccessScopeResource:
		if scopeID == "" || resourceType == "" {
			return errors.New("resource scope requires scopeId and resourceType")
		}
	default:
...
func isLastActiveGlobalAdministrator(tx *gorm.DB, targetUserID uint) (bool, error) {
	...
	if err := tx.Table("rbac_role_bindings AS b").
		Joins("JOIN rbac_roles r ON r.id = b.role_id").
		Where("b.user_id = ? AND r.key = ? AND b.scope_type = ? AND b.scope_id = ?", targetUserID, rbac.RoleAdministrator, model.AccessScopeGlobal, "*").
	...
	if err := tx.Table("rbac_role_bindings AS b").
		Distinct("b.user_id").
		Joins("JOIN rbac_roles r ON r.id = b.role_id").
		Joins("JOIN rbac_users u ON u.id = b.user_id").
		Where("r.key = ? AND b.scope_type = ? AND b.scope_id = ? AND u.status = ?", rbac.RoleAdministrator, model.AccessScopeGlobal, "*", model.AccessUserStatusActive).
		Pluck("b.user_id", &adminIDs).Error; err != nil {
		return false, err
	}
