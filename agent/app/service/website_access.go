...
func PageWebsitesForRBAC(req request.WebsiteSearch, allowedIDs []uint) (int64, []response.WebsiteRes, error) {
	if len(allowedIDs) == 0 { return 0, []response.WebsiteRes{}, nil }
	...
	if req.Name != "" {
		...
		var websiteIDs []uint
		for _, domain := range domains { websiteIDs = append(websiteIDs, domain.WebsiteID) }
		opts = append(opts, websiteRepo.WithSearchKeyword(req.Name, websiteIDs))
	}
	if req.WebsiteGroupID != 0 { opts = append(opts, websiteRepo.WithGroupID(req.WebsiteGroupID)) }
	if req.Type != "" { opts = append(opts, websiteRepo.WithType(req.Type)) }

	total, websites, err := websiteRepo.Page(req.Page, req.PageSize, opts...)
	if err != nil { return 0, nil, err }
	allowed := make(map[uint]struct{}, len(allowedIDs))
	for _, id := range allowedIDs { allowed[id] = struct{}{} }
	...
	for _, web := range websites {
		...
		case constant.Deployment:
			...
			if err == nil { appName = appInstall.Name; appInstallID = appInstall.ID }
		case constant.Runtime:
			runtime, _ := runtimeRepo.GetFirst(context.Background(), repo.WithByID(web.RuntimeID))
			if runtime != nil { runtimeName = runtime.Name; runtimeType = runtime.Type }
		}

		siteDTO := response.WebsiteRes{
			ID: web.ID, CreatedAt: web.CreatedAt, Protocol: web.Protocol, PrimaryDomain: web.PrimaryDomain,
			Type: web.Type, Remark: web.Remark, Status: web.Status, Alias: web.Alias, AppName: appName,
			ExpireDate: web.ExpireDate, SSLExpireDate: web.WebsiteSSL.ExpireDate, SSLStatus: checkSSLStatus(web.WebsiteSSL.ExpireDate),
			RuntimeName: runtimeName, SitePath: GetSitePath(web, SiteDir), AppInstallID: appInstallID,
			RuntimeType: runtimeType, Favorite: web.Favorite, IPV6: web.IPV6,
		}
		...
		if siteDTO.Type == constant.Subsite && web.ParentWebsiteID != 0 {
			if _, ok := allowed[web.ParentWebsiteID]; ok {
				...
				if parentWeb.ID != 0 { siteDTO.ParentSite = parentWeb.PrimaryDomain }
			}
		}
		children, _ := websiteRepo.List(websiteRepo.WithParentID(web.ID), repo.WithByIDs(allowedIDs))
		for _, child := range children { siteDTO.ChildSites = append(siteDTO.ChildSites, child.PrimaryDomain) }
...
func GetWebsitesForRBAC(allowedIDs []uint) ([]response.WebsiteDTO, error) {
	if len(allowedIDs) == 0 { return []response.WebsiteDTO{}, nil }
	...
	for _, web := range websites {
		web.WebsiteSSL = publicWebsiteSSLMetadata(web.WebsiteSSL)
		result = append(result, response.WebsiteDTO{Website: web})
	}
	return result, nil
}

// publicWebsiteSSLMetadata deliberately reconstructs the TLS object from a
// small public allow-list instead of attempting to blank a growing deny-list.
// New WebsiteSSL fields therefore fail closed and cannot accidentally expose
// private keys, PEM, ACME/DNS credentials, shell hooks or host paths.
func publicWebsiteSSLMetadata(source model.WebsiteSSL) model.WebsiteSSL {
	return model.WebsiteSSL{
...
func GetWebsiteOptionsForRBAC(req request.WebsiteOptionReq, allowedIDs []uint) ([]response.WebsiteOption, error) {
	if len(allowedIDs) == 0 { return []response.WebsiteOption{}, nil }
	...
	for _, option := range options {
		if _, ok := allowed[option.ID]; ok { filtered = append(filtered, option) }
	}
	return filtered, nil
}
