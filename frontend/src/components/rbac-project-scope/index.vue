<template>
    <div v-if="visible" class="rbac-project-scope">
        <div class="scope-copy">
            ...
            <span class="scope-hint">Les nouvelles ressources seront rattachées à ce projet.</span>
        </div>
        <el-select
...
const loadProjects = async () => {
    if (!globalStore.isLogin || globalStore.isAdmin || loading.value) {
        return;
    }
    loading.value = true;
    try {
        ...
        const storedStillValid = active.some((project) => project.id === stored);
        if (storedStillValid) {
