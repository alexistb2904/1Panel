...
func collectRestrictedFilePaths(c *gin.Context) ([]string, error) {
	if c.Request.Method == http.MethodGet {
		...
		if value := strings.TrimSpace(c.Query("path")); value != "" {
	...
	if strings.HasPrefix(contentType, "multipart/form-data") {
		...
		for _, key := range []string{"path", "dst", "dir", "outputPath"} {
			if value := strings.TrimSpace(c.Request.FormValue(key)); value != "" {
				paths = append(paths, value)
			}
		}
		return compactPaths(paths), nil
	...
	appendString := func(key string) {
		if value := strings.TrimSpace(valueString(payload[key])); value != "" {
			paths = append(paths, value)
		}
	}
	appendStrings := func(key string) {
		paths = append(paths, valueStrings(payload[key])...)
	}
	for _, key := range []string{"path", "dst", "oldName", "newPath", "outputPath", "dir", "linkPath"} {
		appendString(key)
	}
	for _, key := range []string{"paths", "files", "oldPaths", "coverPaths"} {
		appendStrings(key)
	}
	if oldName := strings.TrimSpace(valueString(payload["oldName"])); oldName != "" {
		if newName := strings.TrimSpace(valueString(payload["newName"])); newName != "" {
			if filepath.IsAbs(newName) {
				...
				paths = append(paths, filepath.Join(filepath.Dir(oldName), newName))
			}
		}
	}
	if dst := strings.TrimSpace(valueString(payload["dst"])); dst != "" {
		if name := strings.TrimSpace(valueString(payload["name"])); name != "" {
			paths = append(paths, filepath.Join(dst, name))
		}
	}
	if path := strings.TrimSpace(valueString(payload["path"])); path != "" {
		if name := strings.TrimSpace(valueString(payload["name"])); name != "" && (strings.HasSuffix(c.Request.URL.Path, "/wget") || strings.HasSuffix(c.Request.URL.Path, "/chunkdownload")) {
	...
	if rawFiles, ok := payload["files"].([]any); ok {
		for _, raw := range rawFiles {
			...
			if input != "" {
				if filepath.IsAbs(input) || base == "" {
					paths = append(paths, input)
				} else {
					paths = append(paths, filepath.Join(base, input))
				}
			}
		}
	}
