...
func RuntimeRBAC() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			denyRuntimeAccess(c, "Unable to parse runtime authorization target")
			return
		}
		if body != nil {
		...
		if err != nil || !ok || key == "" {
			denyRuntimeAccess(c, "Runtime operation is not available to restricted identities")
			return
		}
		...
		// Scoped runtime authorization uses only the canonical runtime:<name>
		// identity resolved from the real Agent row. Historical numeric IDs are
		// deliberately not accepted as a fallback because row IDs are lifecycle
		// artifacts and can become stale/reused after deletion or import.
		...
		if creating || c.Request.URL.Path == "/api/v2/runtimes/update" || c.Request.URL.Path == "/api/v2/runtimes/php/container/update" {
			if err := validateRestrictedRuntimePaths(payload, c.GetHeader(headerRBACProjectRoot)); err != nil {
				denyRuntimeAccess(c, err.Error())
				return
			}
		}
		c.Next()
...
func runtimeIDFromPayloadOrPath(path string, payload map[string]any) uint {
	...
	for i := len(parts) - 1; i >= 0; i-- {
		if id, err := strconv.ParseUint(parts[i], 10, 64); err == nil && id > 0 {
			return uint(id)
		}
	}
	return 0
...
func validateRestrictedRuntimePaths(payload map[string]any, projectRoot string) error {
	...
	if err != nil || !info.IsDir() {
		return &runtimePolicyError{"Project root must exist and be a directory"}
	}
	if codeDir != "" {
	...
	if hasVolumes {
		for _, raw := range rawVolumes {
			...
			if !ok {
				return &runtimePolicyError{"Invalid runtime volume definition"}
			}
			source := strings.TrimSpace(valueString(volume["source"]))
			if source == "" {
				continue
			}
			if err := ensurePathInsideRuntimeRoot(root, source); err != nil {
				return err
			}
		}
	}
...
func ensurePathInsideRuntimeRoot(root, target string) error {
	...
	if err != nil {
		return &runtimePolicyError{"Invalid runtime host path"}
	}
