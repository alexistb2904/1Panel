...
func (s *AccessService) ListNodes() ([]dto.AccessNodeInfo, error) {
	...
	if err := global.DB.Order("name ASC").Find(&rows).Error; err != nil {
		return nil, err
	}
	items := make([]dto.AccessNodeInfo, 0, len(rows))
	for _, row := range rows {
		items = append(items, dto.AccessNodeInfo{ID: row.ID, ExternalKey: row.ExternalKey, Name: row.Name, Status: row.Status, CreatedAt: row.CreatedAt})
	}
	return items, nil
}
...
func (s *AccessService) UpdateNode(req dto.AccessNodeUpdate) error {
	return global.DB.Model(&model.AccessNodeScope{}).Where("id = ?", req.ID).Updates(map[string]any{
		"external_key": strings.TrimSpace(req.ExternalKey), "name": strings.TrimSpace(req.Name), "status": req.Status,
	}).Error
}

func (s *AccessService) ListServiceAccounts() ([]dto.AccessServiceAccountInfo, error) {
	var credentials []model.AccessServiceCredential
	if err := global.DB.Order("name ASC").Find(&credentials).Error; err != nil {
		return nil, err
	}
	...
	for _, credential := range credentials {
		...
		items = append(items, dto.AccessServiceAccountInfo{
			ID: credential.ID, UserID: credential.UserID, Name: credential.Name, KeyID: credential.KeyID,
			IPWhiteList: credential.IPWhiteList, Status: credential.Status, ExpiresAt: credential.ExpiresAt,
			LastUsedAt: credential.LastUsedAt, CreatedAt: credential.CreatedAt, Bindings: bindings,
		})
	}
	return items, nil
}
...
func (s *AccessService) CreateServiceAccount(req dto.AccessServiceAccountCreate) (dto.AccessServiceAccountToken, error) {
	...
	err = global.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := validateServiceAccountBindings(tx, bindings); err != nil {
			return err
		}
...
func (s *AccessService) UpdateServiceAccount(req dto.AccessServiceAccountUpdate) error {
	return global.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.First(&credential, req.ID).Error; err != nil {
			return err
		}
		bindings, err := validateBindings(tx, req.Bindings)
		if err != nil {
		...
		if err := tx.Model(&credential).Updates(map[string]any{"name": strings.TrimSpace(req.Name), "ip_white_list": strings.TrimSpace(req.IPWhiteList), "expires_at": req.ExpiresAt, "status": req.Status}).Error; err != nil {
			return err
		}
		if err := tx.Model(&model.AccessUser{}).Where("id = ?", credential.UserID).Updates(map[string]any{"display_name": strings.TrimSpace(req.Name), "status": req.Status}).Error; err != nil {
			return err
		}
		if err := tx.Where("user_id = ?", credential.UserID).Delete(&model.AccessRoleBinding{}).Error; err != nil {
			return err
		}
		for _, binding := range bindings {
...
func (s *AccessService) RotateServiceAccount(req dto.AccessServiceAccountRotate) (dto.AccessServiceAccountToken, error) {
	...
	var credential model.AccessServiceCredential
	if err := global.DB.First(&credential, req.ID).Error; err != nil {
		return result, err
	}
	if err := global.DB.Model(&credential).Updates(map[string]any{"key_id": keyID, "secret_hash": hash, "last_used_at": nil}).Error; err != nil {
		return result, err
	}
...
func (s *AccessService) SearchAudit(req dto.AccessAuditSearch) (int64, []dto.AccessAuditInfo, error) {
	...
	if req.Decision != "" {
		db = db.Where("decision = ?", req.Decision)
	}
	if req.SubjectType != "" {
	...
	if err := db.Order("id DESC").Limit(req.PageSize).Offset((req.Page - 1) * req.PageSize).Find(&rows).Error; err != nil {
		return 0, nil, err
	}
...
func loadAccessBindings(userID uint) ([]dto.AccessBindingInfo, error) {
	...
	for _, item := range rows {
		items = append(items, dto.AccessBindingInfo{ID: item.ID, RoleID: item.RoleID, RoleKey: item.RoleKey, RoleName: item.RoleName, ScopeType: item.ScopeType, ScopeID: item.ScopeID, ResourceType: item.ResourceType})
	}
	return items, nil
}
...
func validateServiceAccountBindings(tx *gorm.DB, bindings []model.AccessRoleBinding) error {
	for _, binding := range bindings {
		...
		if role.Key == rbac.RoleAdministrator {
			return errors.New("service accounts cannot receive the Administrator role")
		}
	}
	return nil
}
