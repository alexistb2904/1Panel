...
func TestSplitServiceToken(t *testing.T) {
	...
	keyID, gotSecret, ok := splitServiceToken("1ps_0123456789abcdef_" + secret)
	if !ok {
		t.Fatal("expected exact generated service token shape to be valid")
	}
	if keyID != "0123456789abcdef" || gotSecret != secret {
		t.Fatalf("unexpected token split: %s / %s", keyID, gotSecret)
	}
	for _, token := range []string{
		"1ps_short_secret",
		"1ps_0123456789abcde_" + secret,
		"1ps_0123456789abcdef_0123456789abcdef",
		"1ps_0123456789ABCDEF_" + secret,
		"1ps_0123456789abcdef_" + secret[:63] + "G",
		"1ps_0123456789abcdef_" + secret + "00",
	} {
		if _, _, ok := splitServiceToken(token); ok {
			t.Fatalf("malformed service token must be rejected: %q", token)
		}
	}
}
...
func TestServiceAccountClientIPIgnoresSpoofedForwardingFromUntrustedPeer(t *testing.T) {
	...
	if got != "198.51.100.20" {
		t.Fatalf("untrusted peer spoofed forwarding headers: got %s", got)
	}
}
...
func TestServiceAccountClientIPAcceptsForwardingFromTrustedPeer(t *testing.T) {
	...
	got := serviceAccountClientIPWithTrustedProxies(c, "10.0.0.0/8")
	if got != "203.0.113.9" {
		t.Fatalf("expected original client IP, got %s", got)
	}
}
...
func TestServiceAccountClientIPWalksTrustedProxyChainRightToLeft(t *testing.T) {
	...
	if got != "198.51.100.7" {
		t.Fatalf("expected first untrusted client hop, got %s", got)
	}
}
